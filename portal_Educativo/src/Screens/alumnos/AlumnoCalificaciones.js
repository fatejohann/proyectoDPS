...
export default function AlumnoCalificaciones() {
  ...
  const fetchActivities = async (subject) => {
    try {
      ...
      for (const activityDoc of activitiesSnapshot.docs) {
        ...
        const notasCollection = collection(activityDoc.ref, 'notas');
        const notasSnapshot = await getDocs(notasCollection);
        const notasData = [];
  
        notasSnapshot.forEach((notaDoc) => {
          const nota = notaDoc.data().valor;
          notasData.push(nota);
        });
  
        activityData.notas = notasData;
  ...
  const renderActivity = ({ item }) => (
    <View style={styles.activityContainer}>
      ...
      <Text style={styles.activityGrade}>Notas:</Text>
      {item.notas.map((nota, index) => (
        <Text key={index} style={styles.activityGrade}>Nota {index + 1}: {nota}</Text>
      ))}
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      <ComponentHeader 
        ...
        descrip="Bienvenido Estudiante, ¿Qué desea realizar?" 
      ...
      {selectedSubject ? (
        <>
          ...
          <Text style={styles.subjectName}>{selectedSubject.nombreMateria}</Text>
          <FlatList
            data={activities}
        ...
        <>
          <Text style={styles.subHeader}>Selecciona la materia para ver tus notas</Text>
          <FlatList
            data={subjects}
